    ...
    public function getAll(Request $request)
    {
        ...
        return datatables($requestItems)
            ...
            ->addColumn('action', function ($requestItem) {
                $processButton = (auth()->user()->is_user || $requestItem->is_completed) ? '' : "<button class='btn btn-success btn-sm' data-bs-toggle='modal' title='Process request item' data-bs-target='#processRequestItemModal' onclick='selectItem({$requestItem->id});'><i class='bx bx-check-square me-0'></i></button>";
